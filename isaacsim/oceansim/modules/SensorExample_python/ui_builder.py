...
###########################stereo camera ###############################
                camera_check_box = CheckBox(
                    "Stereo Camera",
                    default_value=False,
                    tooltip=" Click this checkbox to activate stereo camera",
...
###########################stereo camera ###############################
                stereo_vp_check_box = CheckBox(
                    "Stereo Viewports",
                    default_value=True,
                    tooltip="Ativa dois viewports (esq/dir) para a câmera estéreo",
                    on_click_fn=self._on_stereo_viewports_checkbox_click_fn,
                )
                self.wrapped_ui_elements.append(stereo_vp_check_box)
#######################################################################
...
###########################stereo camera ###############################
        ...
        self._camL = None
        self._camR = None
        self._camL_trans = np.array([0.3, -0.05, 0.1])  # Deslocado para esquerda
        self._camR_trans = np.array([0.3, 0.05, 0.1])   # Deslocado para direita
        self._cam_stereo_focal_length = 21
        # Stereo viewports arrangement toggle
        self._stereo_viewports_enabled = True
...
###########################stereo camera ###############################
        if self._use_stereo_camera:
            ...
            self._camL = UW_Camera(prim_path=robot_prim_path + '/UW_camera',
                                    name='UW_Camera_Left',
                                    resolution=[1920,1080],
                                    translation=self._camL_trans)
            self._camR = UW_Camera(prim_path=robot_prim_path + '/UW_camera',
                                    name='UW_Camera_Right',
                                    resolution=[1920,1080],
                                    translation=self._camR_trans)
            
            self._camL.set_focal_length(0.1 * self._cam_stereo_focal_length)
            self._camR.set_focal_length(0.1 * self._cam_stereo_focal_length)
            self._camL.set_clipping_range(0.1, 100)
            self._camR.set_clipping_range(0.1, 100)
#########################################################################
    ...
    def _reset_scenario(self):
        ...
        self._scenario.setup_scenario(self._rob, self._sonar, self._cam, 
                                    self._DVL, self._baro, self._ctrl_mode,
                                    self._camL, self._camR) 
...
###########################stereo camera ###############################
    def _on_stereo_viewports_checkbox_click_fn(self, model):
        self._stereo_viewports_enabled = model
        print('Stereo viewports: {}. Reload the scene for changes to take effect.'.format('ON' if model else 'OFF'))
########################################################################
